...
int get_handler(struct mg_connection *c, struct mg_http_message *hm)
{
           ...
           mg_http_match_uri(hm, "/videos/"))
  {
    return (send_message(c, 200, JSON, 
        exec_sql("SELECT array_to_json(array_agg(row_to_json(video))) FROM video",
          PGRES_TUPLES_OK), "%s"));
  }
  ...
  {
    return (send_message(c, 200, JSON,
        exec_sql("SELECT array_to_json(array_agg(row_to_json(content_creator))) FROM content_creator",
          PGRES_TUPLES_OK), "%s"));
  }
  else if (mg_http_match_uri(hm, "/videos/*"))
  {
    return (send_message(c, 200, JSON,
        exec_param_sql("SELECT array_to_json(array_agg(row_to_json(video))) FROM video WHERE id=$1",
          PGRES_TUPLES_OK, hm, 1), "%s"));
  }
  else if (mg_http_match_uri(hm, "/channel/*"))
  {
    return (send_message(c, 200, JSON,
